...
void GPIO_Init() {
  ...
  RCGCGPIO |= (1<<1);  // enable clock to PortB
  GPIO_AFSEL_PORTB = (0x3);  // allow alternative function for PB 0&1
  GPIO_DEN_PORTB = (0x3);  // enable digital pins
  GPIO_PCTL_PORTB &= ~(0xF);  // clear
  GPIO_PCTL_PORTB = (1)|(1<<4);  // set alternative function to UART
}

void UART_Init() {
  // STEP 2:
  // Configure the UART Module
  RCGCUART |= (1<<1);  // enable clock to UART Module 1
  ...
  UART_IBRD = IBRD;
  UART_FBRD = FBRD;
  UART_LCRH &= ~(1<<1);  // disable parity
  UART_LCRH &= ~(1<<3);  // 1 stop bit
  UART_LCRH |= (0x3<<5);  // 8 bit word len
  UART_CC &= ~(0xF);  // use system clock
  UART_CTL = (0x1)|(1<<8)|(1<<9);  // enable UART Module
}
